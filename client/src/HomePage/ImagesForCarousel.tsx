import { Images } from "../assets/images/images";

export type ImageCarouselType = {
    path: string;
};

const Carousel: ImageCarouselType[] = [

    { path: Images.PaganiHuayraBC },
    { path: Images.LamborghiniHuracanEvoSpyder },
    { path: Images.LamborghiniSianFKP37 },
    { path: Images.ApexAP0 },
    { path: Images.AstonMartinValhallaConcept },
    { path: Images.LamborghiniSestoElemento },
    { path: Images.FerrariFXXK },
    { path: Images.VandaElectricsDendrobium },
    { path: Images.Porsche911 },
    { path: Images.BugattiChiron },
    { path: Images.McLarenP1 },
    { path: Images.KoenigseggRegera },
    { path: Images.RimacConceptOne },
    { path: Images.FerrariLaFerrari },
    { path: Images.LamborghiniCentenario },
    { path: Images.AstonMartinValkyrie },
    { path: Images.AutomobiliPininfarinaBattista },
    { path: Images.BugattiVeyron164GSVitesse },

];

export default Carousel;
